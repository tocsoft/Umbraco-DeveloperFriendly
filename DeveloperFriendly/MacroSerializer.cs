...
{
    public class MacroSerializer : BaseTypeSyncer
    {

        public MacroSerializer(string rootFolder, DeveloperFriendly.DeveloperFriendlyApplication.SyncMode mode, bool deleteMissingTypes)
            : base(Path.Combine(rootFolder, "Macros"), mode, deleteMissingTypes)
        {
        ...
        {
            Macro.AfterDelete += (s, e) => { action(); };
            Macro.AfterSave += (s, e) => { action(); };
            Macro.New += (s, e) => { action(); };
