...
{
    public class TemplateSerializer : BaseTypeSyncer
    {
        public TemplateSerializer(string rootFolder, DeveloperFriendly.DeveloperFriendlyApplication.SyncMode mode, bool deleteMissingTypes)
            : base(Path.Combine(rootFolder, "Templates"), mode, deleteMissingTypes)
        {
        ...
        {
            Template.AfterDelete += (s,e)=>{action();};
            Template.AfterSave += (s,e)=>{action();};
            Template.New += (s,e)=>{action();};            
         ...
         void UpdateProperties(XDocument xmlDoc, MediaType docType)
        {
            ...
            var props = docType.PropertyTypes;
          ...
          protected override void DumpConfigs()
          {
            ...
            var allDocTypes = Template.GetAllAsList();

            System.Xml.XmlDocument xmlDoc = new System.Xml.XmlDocument();

            //need to check for xml change before between file on drive and exported before overwriting.
            ...
            foreach (var dt in allDocTypes)
            {
                ...
                if (File.Exists(dt.MasterPageFile))
                {
                    var templateTxt = File.ReadAllText(dt.MasterPageFile);

                    doc.Root.Element("Design").Value = templateTxt;
                }
          ...
          protected override Dictionary<string, string> ExpectedConfigs()
          {
              ...
              var allTypes = Template.GetAllAsList();
